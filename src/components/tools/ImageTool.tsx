    ...
    <div>
      ...
      <div className="space-y-6">
        <SectionCard title="上传图片" subtitle="JPG/PNG/WebP 批量，本地 Canvas 压缩不上传">
          ...
          {files.length > 0 && <p className="text-xs font-mono text-neutral-500 mt-2">已选 {files.length} 张，共 {(files.reduce((a, f) => a + f.size, 0) / 1024).toFixed(0)} KB</p>}
          <div className="flex flex-wrap gap-4 mt-4">
            ...
            <label className="flex items-center gap-2 text-xs font-mono text-neutral-400">质量 <input type="range" min={0.1} max={1} step={0.05} value={quality} onChange={(e) => setQuality(parseFloat(e.target.value))} /> {Math.round(quality * 100)}%</label>
            <label className="flex items-center gap-2 text-xs font-mono text-neutral-400">最大宽 <input type="number" value={maxWidth} onChange={(e) => setMaxWidth(parseInt(e.target.value) || 1920)} className="w-20 px-2 py-1 rounded bg-white/[0.06] border border-white/[0.06]" />px</label>
          </div>
          <button onClick={process} disabled={!files.length || processing} className="mt-4 px-5 py-2 rounded-xl bg-white text-black text-sm font-medium disabled:opacity-40 hover:bg-neutral-200">{processing ? "处理中…" : "开始压缩转换"}</button>
              ...
              {results.map((r) => (
                <div key={r.name} className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-white/[0.03] p-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs font-mono text-neutral-500">{(r.origSize / 1024).toFixed(0)}KB → {(r.size / 1024).toFixed(0)}KB {r.size < r.origSize ? `省 ${(((r.origSize - r.size) / r.origSize) * 100).toFixed(0)}%` : ""}</div>
                  </div>
                  <a href={r.url} download={r.name} className="shrink-0 px-3 py-1.5 rounded-lg bg-emerald-500 text-white text-xs">下载</a>
                </div>
              ))}
            </div>
          </SectionCard>
