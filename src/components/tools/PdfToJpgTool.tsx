...
export default function PdfToJpgTool() {
  ...
  const runRender = useCallback(async () => {
    ...
    try {
      ...
      setBusy(false);
      setProgress("");
      setMsg(`已渲染 ${n} 页，可逐页下载或打包 ZIP`);
    } catch (e) {
      setBusy(false);
      setProgress("");
      setErr(e instanceof Error ? e.message : "PDF 渲染组件加载失败，请刷新重试");
    }
  ...
  const downloadZip = useCallback(async () => {
    ...
    try {
      ...
      setBusy(false);
      setProgress("");
      setErr(e instanceof Error ? e.message : "打包 ZIP 失败");
    }
  }, [pages, ext]);
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        <Hint>
          使用 pdf.js 在浏览器本地渲染，文件不上传。JPG 输出自动填充白色背景避免透明区域变黑。大量页面建议降低清晰度以加快速度。
        </Hint>
      </div>
    </div>
  );
